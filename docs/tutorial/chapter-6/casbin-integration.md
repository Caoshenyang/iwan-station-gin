---
title: "Casbin 权限集成"
description: "Casbin 是一个强大的访问控制库，支持多种访问控制模型（ACL、RBAC、ABAC 等）。它不是传统的权限管理系统，而是一个权限引擎。"
---

# Casbin 权限集成

::: tip 阅读建议
Casbin 这一页概念和代码都偏密集。第一次阅读时建议先理解 `Model`、`Policy`、`Adapter` 三个核心概念，再看 GORM 集成和中间件实现，最后回来看权限管理 API。
:::
...
::: details 先建立一个最小心智模型
你可以先把 Casbin 理解成“权限规则计算器”：请求进来时，它会拿着“谁在访问什么资源、做什么动作”去匹配模型和策略，最后返回允许或拒绝。
:::
...
创建 `server/config/rbac_model.conf`：
...
::: info 读图方法
这里的关键不是记住语法，而是记住判断链路：`用户 -> 角色 -> 策略 -> 资源/动作匹配 -> 允许或拒绝`。后面看中间件时，你会发现它本质上就是把 HTTP 请求翻译成这三个输入参数。
:::
...
Casbin 集成完成后，让我们学习「[权限模型设计](./permission-model)」
